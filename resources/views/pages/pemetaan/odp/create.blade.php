...
@section('content')
    <div class="col-sm-12">
        <div class="card">
            <div class="card-body">
                {!! Form::open(['route' => 'pemetaan-odp.store', 'method' => 'post', 'files' => true]) !!}
                    <div class="form-group">
                        <label for="">Perangkat</label>
                        <select class="form-control" id="perangkat" name="id_odp" style="width: 100%;">
                        @if(!$perangkat->first())
                            <option value="">--Perangkat ODP Tidak Tersedia--</option>
                        @endif
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="port" name="port" style="width: 100%;">
...
@section('script')
    ...
    $(function () {
        $('#petaodc').on('change', function () {
            axios.post('{{ route('pemetaan-odp.port') }}', {id: $(this).val()})
            .then(function (response) {
                $('#port').empty();
                
                if (Object.keys(response.data).length !== 0) {
                    $.each(response.data, function (id, core) {
                        $('#port').append(new Option(core, ++id));
                    })
                } else {
                    $('#port').append(new Option("Port Tidak Tersedia", ""));
                }
            });
        });
    });
